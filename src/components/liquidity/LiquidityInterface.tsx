...
export const LiquidityInterface: React.FC<LiquidityInterfaceProps> = ({
  ...
  }, [selectedPosition, mode, liquidityLogic]);
  ...
  return (
    <div className={`max-w-md mx-auto ${className}`}>
      ...
      <div className="bg-[var(--deep-black)] border border-[var(--charcoal)] rounded-2xl p-6 shadow-2xl">
        ...
        {/* Token Input Section (REPLACED: supports add AND remove modes) */}
                    ...
                    <div className="text-sm text-[var(--silver-dark)] flex items-center space-x-2">
                      ...
                      {(liquidityLogic.lpBalance ?? 0n) > 0n && (
                        <button
                          onClick={() =>
                            liquidityLogic.updateLiquidityState({
                              // we use amountA as the LP input (the hook expects amountA for lp removal in your current setup)
      ...
      <MultiTransactionModal
        ...
        transactionType={
          ...
          // ✅ IMPORTANT: Keep showing modal even when idle if we have a success hash
          liquidityLogic.liquidityState.transactionSuccess ? 
        ...
        mainHash={
          liquidityLogic.liquidityState.txHash  // ✅ Always pass if available
        }
        ...
        isMainConfirming={
          (liquidityLogic.liquidityState.currentStep === 'adding' || 
           liquidityLogic.liquidityState.currentStep === 'removing') && 
           !liquidityLogic.liquidityState.txHash  // ✅ Confirming until hash arrives
        }
        isMainSuccess={
         liquidityLogic.liquidityState.transactionSuccess  // ✅ Use the correct flag
        }
