...
export const DepositProjectTokenCard: React.FC<DepositProjectTokenCardProps> = ({
  ...
  const handleInputChange = (value: string) => {
    ...
    try {
      ...
      if (amount <= BigInt(0)) {
        ...
        setInputError(`Exceeds remaining (${projectTokenInfo.formattedRemaining} ${safeTokenSymbol})`)
      }
    } catch {
  ...
  return (
    <div className="max-w-2xl mx-auto bg-[var(--charcoal)] rounded-xl p-4 space-y-4 shadow-lg">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Coins className="h-4 w-4 text-[var(--neon-orange)]" />
          <div>
            <h3 className="text-base font-semibold text-[var(--off-white)]">Deposit Project Tokens</h3>
            <SafeHtml 
        ...
        {projectTokenInfo.isComplete && <CheckCircle2 className="h-5 w-5 text-[var(--success-green)]" />}
      </div>
      ...
      <div className="space-y-2">
        <div className="flex justify-between text-xs">
          ...
          <span className="text-[var(--off-white)] font-medium">{projectTokenInfo.progressPercentage.toFixed(1)}%</span>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 pt-1">
          <div>
            ...
            <p className="text-sm font-medium text-[var(--off-white)]">{formatDisplayNumber(projectTokenInfo.formattedRequired)}</p>
            <p className="text-xs text-[var(--silver-dark)]">{safeTokenSymbol}</p>
          </div>
          <div>
            <p className="text-xs text-[var(--silver-light)]">Deposited</p>
            <p className="text-sm font-medium text-[var(--neon-orange)]">{formatDisplayNumber(projectTokenInfo.formattedDeposited)}</p>
            <p className="text-xs text-[var(--silver-dark)]">{safeTokenSymbol}</p>
          </div>
          <div>
            <p className="text-xs text-[var(--silver-light)]">Remaining</p>
            <p className="text-sm font-medium text-[var(--neon-red)]">{formatDisplayNumber(projectTokenInfo.formattedRemaining)}</p>
            <p className="text-xs text-[var(--silver-dark)]">{safeTokenSymbol}</p>
          </div>
        </div>
      </div>
            ...
            <div className="relative">
              <input
                ...
                className={`w-full px-3 py-2 bg-[var(--jet-black)] border rounded-lg text-[var(--off-white)] text-sm
                  ...
                  ${inputError ? 'border-[var(--neon-orange)] focus:ring-[var(--neon-orange)]/50' : 'border-[var(--silver-dark)]/30 focus:ring-[var(--neon-orange)]/50'}`}
              />
      ...
      {projectTokenInfo.isComplete && (
        <div className="bg-[var(--success-green)]/10 border border-[var(--success-green)]/30 rounded-lg p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-xs font-medium text-[var(--success-green)]">Project Tokens Deposited</p>
              <p className="text-xs text-[var(--silver-light)] mt-0.5">Ready for contributors once project starts.</p>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Info */}
      <div className="bg-[var(--neon-orange)]/5 border border-[var(--neon-orange)]/20 rounded-lg p-2.5">
        <p className="text-xs text-[var(--silver-light)] leading-relaxed">
          💡 Project tokens sold to contributors during funding period. Deposit in multiple transactions if needed.
        </p>
      </div>
    </div>
  )
